...
CREATE OR REPLACE FUNCTION public.fetch_readings_function(p_coa_id uuid)
 RETURNS TABLE(reading_id uuid, certificate_of_analysis_id uuid, sample_time timestamp with time zone, sample_time_formatted text, floor text, area text, location text, outlet text, feed_type text, flush_type text, parameters text, coa_results_list jsonb)
 LANGUAGE sql
 STABLE
AS $function$
with this_coa as (
